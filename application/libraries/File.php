...
class Image {
    ...
    * anh cate   
    */
    function cateImage($imgName='',$config=array()){
        $width      = isset($config['width']) ? $config['width'] : '50';
        $height      = isset($config['height']) ? $config['height'] : '50';
        $default    = isset($config['default']) ? $config['default']: true; // Check cho lấy ảnh mặc định ko (true or false)
        $src        = isset($config['src']) ? $config['src'] : false;   // Có lấy đường dẫn ảnh không (true or false)
        ...
        if($imgName == '')
            $imgName = 'news.gif';            
        $checkFileDefault = $this->checkFile('categories'.DS.$imgName,$width,$height,$default,true);
        if( $checkFileDefault !== true){
            if($checkFileDefault == false){
                return false; 
            }else{
        ...
        $server = $this->server;
        if($width == '')
            $url =  $this->rewriteUrl('categories').DS.$imageName;
        else
            $url =  $this->rewriteUrl('categories').DS.'thumb'.DS.$imageName;
        if($src) 
            return  $url;  
